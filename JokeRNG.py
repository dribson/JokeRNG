...
class JokeR:
    ...
    def __init__(self, root):
        ...
        ttk.Label(mainframe, text="Current Action: ").grid(column=1, row=1, sticky=E)
        ttk.Label(mainframe, textvariable=self.step).grid(column=2, row=1, sticky=(W,E))
        ttk.Button(mainframe, textvariable=self.playbtn, command=self.NextGameState).grid(column=3, row=1, sticky=(W, E))
        ttk.Button(mainframe, text="Calculate Next Action (Blind Won)", command=self.NextGameStateWin).grid(column=4, row=1, sticky=(W, E))
        ttk.Button(mainframe, text="Press to Lose", command=self.Exit).grid(column=5, row=1, sticky=W)

        ttk.Label(mainframe, text="Run Variables").grid(column=1, row=2, sticky=E)
        ttk.Label(mainframe, text="Update these before calculating next action").grid(column=2, row=2, sticky=E)
        ...
        ttk.Label(mainframe, text="Hand Size: ").grid(column=1, row=3, sticky=E)
        ttk.Label(mainframe, textvariable=(self.handSize)).grid(column=2, row=3, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddHandSize).grid(column=3, row=3, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubHandSize).grid(column=4, row=3, sticky=W)

        ttk.Label(mainframe, text="Joker Slots: ").grid(column=1, row=4, sticky=E)
        ttk.Label(mainframe, textvariable=(self.jokerSize)).grid(column=2, row=4, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddJokerSize).grid(column=3, row=4, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubJokerSize).grid(column=4, row=4, sticky=W)

        ttk.Label(mainframe, text="Consumable Slots: ").grid(column=1, row=5, sticky=E)
        ttk.Label(mainframe, textvariable=(self.consumableSize)).grid(column=2, row=5, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddConsumeSize).grid(column=3, row=5, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubConsumeSize).grid(column=4, row=5, sticky=W)

        ttk.Label(mainframe, text="Shop Slots: ").grid(column=1, row=6, sticky=E)
        ttk.Label(mainframe, textvariable=(self.shopSize)).grid(column=2, row=6, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddShopSize).grid(column=3, row=6, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubShopSize).grid(column=4, row=6, sticky=W)

        ttk.Label(mainframe, text="Owned Jokers: ").grid(column=1, row=7, sticky=E)
        ttk.Label(mainframe, textvariable=(self.ownedJokers)).grid(column=2, row=7, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddOwnedJoker).grid(column=3, row=7, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubOwnedJoker).grid(column=4, row=7, sticky=W)

        ttk.Label(mainframe, text="Owned Consumables: ").grid(column=1, row=8, sticky=E)
        ttk.Label(mainframe, textvariable=(self.ownedConsumables)).grid(column=2, row=8, sticky=(W, E))
        ttk.Button(mainframe, text="+", command=self.AddOwnedConsume).grid(column=3, row=8, sticky=(W, E))
        ttk.Button(mainframe, text="-", command=self.SubOwnedConsume).grid(column=4, row=8, sticky=W)


        ttk.Label(mainframe, text="Manually Select Options:").grid(column=1, row=9, sticky=E)
        ttk.Label(mainframe, text="# Items to Pick").grid(column=2, row=9, sticky=(W, E))
        ttk.Label(mainframe, text="# Total Items Available").grid(column=3, row=9, sticky=(W, E))
        ttk.Label(mainframe, text="Output").grid(column=4, row=9, sticky=W)
        ...
        self.packMax.set("")
        ttk.Button(mainframe, text="Select Cards", command=self.GenerateNumbersOfPackCards).grid(column=1, row=10, sticky=E)
        ttk.Entry(mainframe, width=2, textvariable=self.packInput).grid(column=2, row=10, sticky=(W, E))
        ttk.Entry(mainframe, width=2, textvariable=self.packMax).grid(column=3, row=10, sticky=(W, E))
        ttk.Label(mainframe, textvariable=(self.packText)).grid(column=4, row=10, sticky=W)
    ...
    def NextGameState(self, *args):
        ...
        if(self.gameState.get() == 0): # starting a new game
            ...
            if random.randint(1, 100) <= weightSellJoker and self.ownedJokers.get() > 0: # Randomly sell a Joker, if one is owned
    ...
    def PickShopAction(self, *args):
        if(self.actionsShop.get() > 0): # Calculate a random shop action
            ...
            elif randShop == 4 and self.ownedJokers.get() > 0 and random.randint(1, 100) <= weightSellJoker:
                # Sell a Random Joker
                randJoker = random.randint(1, self.ownedJokers.get())
                self.step.set(f"Action {self.currentAction.get()}: Sell the Joker currently in Slot #{randJoker}")
                self.ownedJokers.set(self.ownedJokers.get() - 1)
    ...
    def NextGameStateWin(self, *args):
        if(self.gameState.get() == 1):
            self.currentBlind.set((self.currentBlind.get() + 1) % 3)
